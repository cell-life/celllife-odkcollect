...
public class InstanceProvider extends ContentProvider {
    ...
    private static class DatabaseHelper extends ODKSQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            db.execSQL("CREATE TABLE " + INSTANCES_TABLE_NAME + " (" + InstanceColumns._ID + " integer primary key, "
                    + InstanceColumns.DISPLAY_NAME + " text not null, " + InstanceColumns.SUBMISSION_URI + " text, "
                    + InstanceColumns.CAN_EDIT_WHEN_COMPLETE + " text, " + InstanceColumns.INSTANCE_FILE_PATH
                    + " text not null, " + InstanceColumns.JR_FORM_ID + " text not null, " + InstanceColumns.JR_VERSION
                    + " text, " + InstanceColumns.REFERENCE + " text, " + InstanceColumns.STATUS + " text not null, "
                    + InstanceColumns.LAST_STATUS_CHANGE_DATE + " date not null, " + InstanceColumns.DISPLAY_SUBTEXT
                    + " text not null );");
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion == 1) {
                ...
                db.execSQL("UPDATE " + INSTANCES_TABLE_NAME + " SET " + InstanceColumns.CAN_EDIT_WHEN_COMPLETE + " = '"
                        + Boolean.toString(true) + "' WHERE " + InstanceColumns.STATUS + " IS NOT NULL AND "
                        + InstanceColumns.STATUS + " != '" + InstanceProviderAPI.STATUS_INCOMPLETE + "'");
                oldVersion = 2;
            }
            if (oldVersion == 2) {
                db.execSQL("ALTER TABLE " + INSTANCES_TABLE_NAME + " ADD COLUMN " + InstanceColumns.JR_VERSION
                        + " text;");
            }
            if (oldVersion == 3) {
            ...
            if (oldVersion == 5) {
                ...
                db.execSQL("UPDATE " + INSTANCES_TABLE_NAME + " SET " + InstanceColumns.REFERENCE + " = '" + "-"
                        + "' WHERE " + InstanceColumns.REFERENCE + " IS NULL ");
            }
            if (oldVersion == 9) {

                db.execSQL("UPDATE " + INSTANCES_TABLE_NAME + " SET " + InstanceColumns.REFERENCE + " = '" + ""
                        + "'WHERE " + InstanceColumns.REFERENCE + " = '" + "NULL'");
            }
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        // if (values.containsKey(InstanceColumns.REFERENCE) == false) {
        // String noReference = getContext().getString(R.string.no_reference);
        // values.put(InstanceColumns.REFERENCE, noReference);
        // }
        ...
        if (rowId > 0) {
            ...
            Collect.getInstance()
                    .getActivityLogger()
                    .logActionParam(this, "insert", instanceUri.toString(),
    ...
    private String getDisplaySubtext(String state, Date date) {
        if (state == null) {
            return new SimpleDateFormat(getContext().getString(R.string.added_on_date_at_time), Locale.getDefault())
                    .format(date);
        } else if (InstanceProviderAPI.STATUS_INCOMPLETE.equalsIgnoreCase(state)) {
            return new SimpleDateFormat(getContext().getString(R.string.saved_on_date_at_time), Locale.getDefault())
                    .format(date);
        } else if (InstanceProviderAPI.STATUS_COMPLETE.equalsIgnoreCase(state)) {
            return new SimpleDateFormat(getContext().getString(R.string.finalized_on_date_at_time), Locale.getDefault())
                    .format(date);
    ...
    /**
     * This method removes the entry from the content provider, and also removes any associated files. files: form.xml,
     * [formmd5].formdef, formname-media {directory}
     */
    @Override
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        case INSTANCE_ID:
            ...
            count = db.delete(INSTANCES_TABLE_NAME, InstanceColumns._ID + "=" + instanceId
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        case INSTANCE_ID:
            ...
            count = db.update(INSTANCES_TABLE_NAME, values,
                    InstanceColumns._ID + "=" + instanceId + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""),
                    whereArgs);
            break;
